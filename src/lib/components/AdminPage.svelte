...
<div class="min-h-screen bg-[var(--color-bg)] text-[var(--color-fg)] font-mono p-8 flex flex-col gap-8">
  <div class="flex items-center justify-between">
    <a href="/" class="text-[var(--color-fg-dim)] hover:text-[var(--color-fg)] text-sm">← back</a>
    <form method="POST" action="/auth/logout">
      <button type="submit" class="text-[var(--color-fg-dim)] hover:text-[var(--color-fg)] text-sm">sign out</button>
    </form>
  </div>

  <section class="flex flex-col gap-2">
    <h2 class="text-[var(--color-fg-dim)] text-xs uppercase tracking-widest">creature</h2>
    <div class="flex flex-col gap-1 text-xs text-[var(--color-fg-dim)]">
  ...
  <section class="flex flex-col gap-2">
    <h2 class="text-[var(--color-fg-dim)] text-xs uppercase tracking-widest">generation</h2>
    <button
      onclick={forceGenerate}
      disabled={generating}
      class="text-[var(--color-accent)] hover:opacity-80 disabled:opacity-40 text-sm text-left w-fit"
    >
  ...
  <section class="flex flex-col gap-2">
    <h2 class="text-[var(--color-fg-dim)] text-xs uppercase tracking-widest">frames ({frames.length})</h2>
    {#if frames.length === 0}
      <span class="text-[var(--color-fg-faint)] text-xs">no frames</span>
    {:else}
      <div class="flex flex-col gap-4">
        {#each frames as frame (frame.id)}
          <div class="border border-[var(--color-border)] p-3 flex flex-col gap-2">
            <div class="flex items-start justify-between gap-4">
              <pre class="text-[var(--color-accent)] text-xs leading-tight opacity-80 max-h-32 overflow-hidden">{frame.ascii.split('\n').slice(0, 6).join('\n')}</pre>
              <button
                onclick={() => deleteFrame(frame.id)}
                class="text-[var(--color-fg-faint)] hover:text-[var(--color-danger)] text-xs shrink-0"
              >×</button>
            </div>
            <div class="text-[var(--color-fg-faint)] text-xs flex gap-3 flex-wrap">
              {#each Object.entries(frame.weights) as [k, v]}
                <span>{k.slice(0, 3)}: {(v as number).toFixed(2)}</span>
              {/each}
            </div>
            <span class="text-[var(--color-fg-faint)] text-xs">{new Date(frame.created_at).toISOString().replace('T', ' ').slice(0, 19)}</span>
          </div>
        {/each}
      </div>
    {/if}
  ...
  <section class="flex flex-col gap-3 border-t border-[var(--color-border)] pt-8">
    ...
    <button
      ...
      class="text-[var(--color-danger)] hover:opacity-80 text-sm text-left w-fit border border-[var(--color-danger)] hover:opacity-80 px-3 py-1 transition-colors"
    >
      Delete Account
    </button>
  </section>
