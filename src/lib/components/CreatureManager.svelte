...
<div class="flex flex-col gap-3">
  <div class="flex items-center justify-between">
    <span class="text-[var(--color-fg-dim)] text-xs">{allCreatures.length} / {maxCreatures}</span>
    <button
      onclick={hatch}
      disabled={hatching || allCreatures.length >= maxCreatures}
      class="text-[var(--color-accent-dim)] hover:text-[var(--color-accent)] disabled:opacity-30 text-xs"
    >
  ...
  {#each allCreatures as c (c.id)}
    <div class="flex items-start gap-2 text-xs">
      ...
      <div class="flex flex-col gap-0.5 flex-1 min-w-0">
        <span class="text-[var(--color-fg)] font-mono">{c.id.slice(0, 8)}</span>
        <span class="text-[var(--color-fg-dim)]">{age(c.created_at)} · {c.frames.length} frames</span>
      </div>
      {#if c.display_order !== 0}
        <button
          onclick={() => deleteCreature(c.id)}
          class="text-[var(--color-fg-faint)] hover:text-[var(--color-danger)] shrink-0"
        >×</button>
      {/if}
    </div>
  {/each}
